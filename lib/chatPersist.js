...
const VERSION     = 6;
const MIN_VERSION = 5; // accept data saved with v5 or newer
...
export function loadPersistedState() {
  try {
    const data = read();
    if (!data) return null;
    // Accept any version >= MIN_VERSION so code updates don't wipe live sessions.
    if (!data.v || data.v < MIN_VERSION) return null;
