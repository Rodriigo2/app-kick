...
function ensurePersistSetup() {
  if (typeof window !== "undefined" && !G.__persistListenerAdded) {
    ...
    // Save when tab becomes hidden (more reliable than beforeunload on mobile/some browsers)
    document.addEventListener("visibilitychange", () => {
      if (document.hidden && S.total > 0) persistState(S);
    });

    // Save periodically as insurance (every 30s regardless of flush cycle)
    setInterval(() => {
      if (S.status === "connected" && S.total > 0) persistState(S);
    }, 30_000);
  }
}
...
export async function connect(input) {
  ...
  if (!isSameChannel) {
    // Fresh channel — wipe accumulated data.
    S.usersMap      = new Map();
